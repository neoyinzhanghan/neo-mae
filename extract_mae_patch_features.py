...
parser = argparse.ArgumentParser(description='Extracts patch features from a WSI. Assumes you have already created the patch grid.')
...
group.add_argument("--wsi_dir", type=str,
                   help="Directory where the WSI files live.")

group.add_argument("--patch_dir", type=str,
...
parser.add_argument("--is_dpr", default=False, action="store_true",
...
parser.add_argument("--max_n_wsis", default=None, type=int,
                    help="Maximum number of WSIs to process. Mainly for debugging/prototyping.")


parser.add_argument("--max_n_patches", default=None, type=int,
...
group.add_argument("--guess_largest_batch_size",
...
wsi_fpaths, bad_fpths_no_mpp = find_wsi_paths(folder=args.wsi_dir,
                                              recursive=False,
                                              skip_missing_mpp=True,
                                              max_n_wsis=args.max_n_wsis)

print("Found {} WSIs".format(len(wsi_fpaths)))
print("{} WSIs were missing mpp values: {}".
      format(len(bad_fpths_no_mpp), bad_fpths_no_mpp))


wsi_fpaths, wsi_fpaths_missing_patch, _ = \
    find_fpaths_with_matching_folder(wsi_fpaths=wsi_fpaths,
                                     super_folder=args.patch_dir)

print("Found {} WSIs with patches, {} missing patches,\n{}".
      format(len(wsi_fpaths),
             len(wsi_fpaths_missing_patch),
             wsi_fpaths_missing_patch))
...
if args.guess_largest_batch_size:
    ...
    start_time = time()
    best_batch_guess, try_info, other_info = \
        guess_largest_batch_size(model=model,
                                 mode='extract',
                                 x=x,
                                 max_n_tries=10,
    ...
    print("Guessing the best batch size took {} seconds".
          format(guest_batch_size_runtime))
...
for wsi_fpath in tqdm(wsi_fpaths):
    ...
    wsi_feats_save_fpath = \
        os.path.join(args.save_dir, '{}-patch_features.h5'.format(wsi_name))
    wsi_patch_folder = os.path.join(args.patch_dir, wsi_name)

    # load already computed patch grid
    patch_coords, patch_grid_idx = \
        load_patch_grid_coords(folder=wsi_patch_folder,
                               max_n=args.max_n_patches)
    ...
    patches = get_several_patches_at_mpp_ray(wsi_fpath=wsi_fpath,
    ...
    output = transformed_ds.\
        map_batches(fn=FeatureExtractor,
    ...
    #########################
    start_time = time()
    add_array_batch_to_h5(fpath=wsi_feats_save_fpath,
                          data=features, name='features')
    add_array_batch_to_h5(fpath=wsi_feats_save_fpath,
                          data=patch_coords, name='coords')
    add_array_batch_to_h5(fpath=wsi_feats_save_fpath,
                          data=patch_grid_idx, name='patch_grid_idx')
    ...
    ###############################
    process_info = {'wsi_name': wsi_name,
    ...
    pd.DataFrame([process_info]).\
        to_csv(process_info_fpath,
               mode='a',
